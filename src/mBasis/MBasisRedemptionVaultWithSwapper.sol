 ...
 */
contract MBasisRedemptionVaultWithSwapper is IMBASISRedemptionVaultWithSwapper, MBasisRedemptionVault {
        ...
        onlyNotSanctioned(msg.sender)
    {
        ...
        (uint256 feeAmount, uint256 amountMTokenWithoutFee) =
        ...
        (uint256 amountMTokenInUsd, uint256 mTokenRate) = _convertMTokenToUsd( // @audit-info mBASIS --> USD
        amountMTokenInCopy);

        (uint256 amountTokenOut, uint256 tokenOutRate) = _convertUsdToToken( // @audit-info USD --> USDC
        amountMTokenInUsd, tokenOutCopy);

        uint256 amountTokenOutWithoutFee = _truncate( // Obtain amount of `tokenOut` without the fee
            (amountMTokenWithoutFee * mTokenRate) / tokenOutRate, // @audit-info mBASIS --> USD --> USDC (truncate in the end)
            tokenDecimals
        );

        require(
            amountTokenOutWithoutFee >= minReceiveAmountCopy, // @audit-ok [HIGH-02] - Wrong slippage check in `MBasisRedemptionVaultWithSwapper` leads to user getting less funds than expected.
            "RVS: minReceiveAmount > actual" // ^^^^^^^^^ Below it has a slippage check when redeeming when calling `redeemInstant`!!!
        );
        ...
        uint256 contractTokenOutBalance = IERC20(tokenOutCopy).balanceOf(address(this));
        ...
        if (contractTokenOutBalance >= amountTokenOutWithoutFee.convertFromBase18(tokenDecimals)) {
            ...
            IERC20(mTbillRedemptionVault.mToken()).safeIncreaseAllowance(address(mTbillRedemptionVault), mTbillAmount);

            mTbillRedemptionVault.redeemInstant( // Exchange mTBILL for USDC by redeeming in mTBILL's Midas redemption vault
            tokenOutCopy, mTbillAmount, minReceiveAmountCopy);

            uint256 contractTokenOutBalanceAfterRedeem = IERC20(tokenOutCopy).balanceOf(address(this));

            // Compute final balance of tokenOut after swapping and redeeming mTBILL
            amountTokenOutWithoutFee =
                (contractTokenOutBalanceAfterRedeem - contractTokenOutBalance).convertToBase18(tokenDecimals);
        }
     ...
     */
    function _swapMBasisToMToken(uint256 mBasisAmount) internal returns (uint256 mTokenAmount) {
        _tokenTransferFromUser( // Transfer mBASIS
        address(mToken), liquidityProvider, mBasisAmount, 18);

        uint256 mTbillRate = mTbillRedemptionVault.mTokenDataFeed() // Fetch mTBILL rate
            .getDataInBase18();

        uint256 mTokenRate = mTokenDataFeed.getDataInBase18(); // Fetch mBASIS rate

        mTokenAmount = (mBasisAmount * mTokenRate) / mTbillRate; // Convert mBASIS to mTBILL corresponding amount

        _tokenTransferFromTo( // Transfer mtBILL from provider to this contract ("swap")
        address(mTbillRedemptionVault.mToken()), liquidityProvider, address(this), mTokenAmount, 18);
    }
}
