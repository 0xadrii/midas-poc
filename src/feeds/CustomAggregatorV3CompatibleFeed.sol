 ...
 */
abstract contract CustomAggregatorV3CompatibleFeed is WithMidasAccessControl, AggregatorV3Interface {
    struct RoundData {
        uint80 roundId;
        int256 answer;
        uint256 startedAt;
    ...
    function initialize(
        ...
        require(_maxAnswerDeviation <= 100 * (10 ** decimals()), "CA: !max deviation");
    ...
    function setRoundData(int256 _data) public onlyAggregatorAdmin {
        require(_data >= minAnswer && _data <= maxAnswer, "CA: out of [min;max]");
        ...
        returns (uint80 roundId, int256 answer, uint256 startedAt, uint256 updatedAt, uint80 answeredInRound)
    {
        ...
        return
            (roundData.roundId, roundData.answer, roundData.startedAt, roundData.updatedAt, roundData.answeredInRound);
    }
