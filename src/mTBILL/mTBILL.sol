...
contract mTBILL is ERC20PausableUpgradeable, Blacklistable, IMTbill {
     ...
     * @inheritdoc IMTbill
     */
    function mint(address to, uint256 amount) external onlyRole(_minterRole(), msg.sender) {
        _mint(to, amount);
    }
     ...
     * @inheritdoc IMTbill
     */
    function burn(address from, uint256 amount) external onlyRole(_burnerRole(), msg.sender) {
        _burn(from, amount);
    }
     ...
     * @inheritdoc IMTbill
     */
    function setMetadata(bytes32 key, bytes memory data) external onlyRole(DEFAULT_ADMIN_ROLE, msg.sender) {
        metadata[key] = data;
    }
