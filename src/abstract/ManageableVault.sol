 ...
 */
abstract contract ManageableVault is Pausable, IManageableVault, Blacklistable, Greenlistable, WithSanctionsList {
     ...
     */
    function withdrawToken(address token, uint256 amount, address withdrawTo) external onlyVaultAdmin {
        IERC20(token).safeTransfer(withdrawTo, amount); // @audit-issue [MEDIUM-05] - Admin could be unable to recover BUIDL tokens from the contract due to incorrect logic in withdrawToken
     ...
     */
    function addPaymentToken(address token, address dataFeed, uint256 tokenFee, bool stable) external onlyVaultAdmin {
        require(_paymentTokens.add(token), "MV: already added");
        _validateAddress(dataFeed, false);
        _validateFee(tokenFee, false);

        tokensConfig[token] = TokenConfig({dataFeed: dataFeed, fee: tokenFee, allowance: MAX_UINT, stable: stable});
     ...
     */
    function changeTokenFee(address token, uint256 fee) external onlyVaultAdmin {
        _requireTokenExists(token);
     ...
     * @param tokenDecimals token decimals
     */
    function _tokenTransferFromTo(address token, address from, address to, uint256 amount, uint256 tokenDecimals)
        internal
    {
        uint256 transferAmount = amount.convertFromBase18(tokenDecimals);
     ...
     */
    function _getFeeAmount(address sender, address token, uint256 amount, bool isInstant, uint256 additionalFee)
        internal
        view
        returns (uint256)
    {
    ...
    function _requireVariationTolerance(uint256 prevPrice, uint256 newPrice) internal view {
        ...
        uint256 priceDifPercent = (priceDif * ONE_HUNDRED_PERCENT) / prevPrice; // @audit-ok Variation tolerance checks look good with a 0.01% granularity (BPS)
