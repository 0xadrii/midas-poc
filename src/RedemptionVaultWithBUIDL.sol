...
contract RedemptionVaultWIthBUIDL is RedemptionVault {
        ...
        onlyNotSanctioned(msg.sender)
    {
        ...
        uint256 amountTokenOutWithoutFee = (
            amountMTokenWithoutFee // mTBILL --> USD --> USDC
                * mTokenRate
        ) / tokenOutRate;

        require(
            amountTokenOutWithoutFee >= minReceiveAmountCopy, // @audit-ok Even if the check is performed before truncation, it is correct as the decimal difference is NOT reflected in the actual amount of USDC returned
        ...
        _tokenTransferToUser(
            tokenOutCopy, user, amountTokenOutWithoutFeeFrom18.convertToBase18(tokenDecimals), tokenDecimals
        );
    ...
    function _checkAndRedeemBUIDL(address tokenOut, uint256 amountTokenOut) internal {
        ...
        buidlRedemption.redeem(buidlToRedeem); // @audit-issue [MEDIUM-03] - Not considering BUIDL minimum balances and full balance transfer configurations can lead to DoS
    }
}
