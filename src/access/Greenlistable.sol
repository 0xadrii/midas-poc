...
abstract contract Greenlistable is WithMidasAccessControl {
     ...
     */
    bytes32 public constant GREENLIST_TOGGLER_ROLE = keccak256("GREENLIST_TOGGLER_ROLE");
    ...
    // @audit-issue [MEDIUM-01] - Missing storage gap might lead to storage corruption
    ...
    function __Greenlistable_init(address _accessControl) internal onlyInitializing {
        __WithMidasAccessControl_init(_accessControl);
        __Greenlistable_init_unchained(); // @audit-ok Don't think it's worth adding the fact that greenlist can't be initialized at the beginning
    }
     ...
     */
    function _onlyGreenlisted(address account) private view onlyRole(greenlistedRole(), account) {}

    /**
     * @dev checks that a given `account`
     * have a `greenlistTogglerRole()`
     */
    function _onlyGreenlistToggler(address account) internal view onlyRole(greenlistTogglerRole(), account) {}
}
