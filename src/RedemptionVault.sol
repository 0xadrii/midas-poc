...
contract RedemptionVault is ManageableVault, IRedemptionVault {
        ...
        onlyNotSanctioned(msg.sender)
    {
        ...
        (uint256 amountMTokenInUsd, uint256 mTokenRate) = _convertMTokenToUsd(amountMTokenInCopy);

        (uint256 amountTokenOut, uint256 tokenOutRate) = _convertUsdToToken(amountMTokenInUsd, tokenOutCopy);

        uint256 amountTokenOutWithoutFee = _truncate(
            (amountMTokenWithoutFee * mTokenRate) / tokenOutRate, // @audit-info Conversion: `mToken` --> USD --> `tokenOut`
            tokenDecimals
        );

        require(amountTokenOutWithoutFee >= minReceiveAmountCopy, "RV: minReceiveAmount > actual");

        _requireAndUpdateAllowance(tokenOutCopy, amountTokenOut); // @audit-issue [MEDIUM-04] - Allowance is incorrectly updated when redeeming
     ...
     * @inheritdoc IRedemptionVault
     */
    function safeApproveRequest(uint256 requestId, uint256 newMTokenRate) external onlyVaultAdmin {
    ...
    function _approveRequest(uint256 requestId, uint256 newMTokenRate, bool isSafe) internal {
        ...
        if (request.tokenOut != MANUAL_FULLFILMENT_TOKEN) {
            ...
            uint256 amountTokenOutWithoutFee =
                _truncate((request.amountMToken * newMTokenRate) / request.tokenOutRate, tokenDecimals);

            _tokenTransferFromTo(
                request.tokenOut, requestRedeemer, request.sender, amountTokenOutWithoutFee, tokenDecimals
            );
        }
     ...
     */
    function _validateRequest(address sender, RequestStatus status) internal pure {
     ...
     */
    function _redeemRequest(address tokenOut, uint256 amountMTokenIn) internal returns (uint256) {
        address user = msg.sender;

        bool isFiat = tokenOut == MANUAL_FULLFILMENT_TOKEN;

        (uint256 feeAmount, uint256 amountMTokenWithoutFee) =
            _calcAndValidateRedeem(user, tokenOut, amountMTokenIn, false, isFiat);
     ...
     */
    function _calcAndValidateRedeem(address user, address tokenOut, uint256 amountMTokenIn, bool isInstant, bool isFiat)
        internal
        view
        returns (uint256 feeAmount, uint256 amountMTokenWithoutFee)
    {
        ...
        if (isFiat) {
            require(tokenOut == MANUAL_FULLFILMENT_TOKEN, "RV: tokenOut != fiat");
            if (!waivedFeeRestriction[user]) feeAmount += fiatFlatFee; // @audit-info Not a percentage!!
        } else {
