...
contract DepositVault is ManageableVault, IDepositVault {
        ...
        onlyNotSanctioned(msg.sender)
    {
        ...
        require(mintAmount >= minReceiveAmount, "DV: minReceiveAmount > actual");

        totalMinted[user] += mintAmount;

        _requireAndUpdateLimit(mintAmount); // Update instant daily limit

        _tokenTransferFromUser(tokenInCopy, tokensReceiver, amountTokenWithoutFee, tokenDecimals);

        if (
            feeTokenAmount > 0 // @audit-info Note that this fee **is already truncated**!!
        ) {
            _tokenTransferFromUser(tokenInCopy, feeReceiver, feeTokenAmount, tokenDecimals);
        }
        ...
        emit DepositInstant(
            user, tokenInCopy, tokenAmountInUsd, amountTokenCopy, feeTokenAmount, mintAmount, referrerIdCopy
        );
    }
        ...
        returns (uint256 requestId)
    {
        ...
        uint256 currentId = currentRequestId;
        requestId = currentId;
        currentRequestId++;
        ...
        if (feeAmount > 0) {
            _tokenTransferFromUser(tokenInCopy, feeReceiver, feeAmount, tokenDecimals);
        }
        ...
        mintRequests[currentId] = Request({
            ...
            usdAmountWithoutFees: (amountTokenWithoutFee * tokenInRate) / 10 ** 18,
            tokenOutRate: tokenOutRate
        });

        emit DepositRequest(currentId, user, tokenInCopy, tokenAmountInUsd, feeAmount, tokenOutRate, referrerIdCopy);
    }
     ...
     * @inheritdoc IDepositVault
     */
    function setMinMTokenAmountForFirstDeposit(uint256 newValue) external onlyVaultAdmin {
     ...
     */
    function _approveRequest(uint256 requestId, uint256 newOutRate, bool isSafe) private {
        Request memory request = mintRequests[requestId];
        ...
        mToken.mint(request.sender, amountMToken); // @audit-issue [HIGH-01] - Lack of sanctioned user checks allows users who become sanctioned between deposit/redeem request and approval to still get their requests approved

        totalMinted[request.sender] += amountMToken; // @audit Shouldn't minimums be rechecked here?
    ...
    function _calcAndValidateDeposit(
        address user,
        address tokenIn,
        uint256 amountToken, // @audit-info This is intended to have 18 decimals always
        bool isInstant
    )
        internal
        returns (
        ...
        )
    {
        ...
        (uint256 amountInUsd, uint256 tokenInUSDRate) = _convertTokenToUsd( // Convert token in to USD
        tokenIn, amountToken);
        ...
        _requireAndUpdateAllowance(tokenIn, amountToken); // reduce total allowance (in `tokenOut`)

        feeTokenAmount = _truncate(
            _getFeeAmount(userCopy, tokenIn, amountToken, isInstant, 0), // Apply percentage to `amountToken`
            tokenDecimals
        );
        ...
        returns (uint256 amountInUsd, uint256 rate)
    {
        ...
        rate = _getTokenRate(tokenConfig.dataFeed, tokenConfig.stable); // @audit-info Rate is returned in 18 decimals
     ...
     */
    function _convertUsdToMToken(uint256 amountUsd) internal view returns (uint256 amountMToken, uint256 mTokenRate) {
